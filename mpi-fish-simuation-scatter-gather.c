...
//        free(all_fishes);
//    } else {
//        MPI_Recv(local_fishes, localFishCount, MPI_FISH_TYPE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//    }
//
...
//        MPI_Allreduce(&localDistanceSum, &globalDistanceSum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
//
...
//            } else {
//                MPI_Recv(all_fishes + offset, fishToReceive, MPI_FISH_TYPE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//            }
...
// ... [rest of the includes and defines remain unchanged]

// ... [rest of the utility functions and structs remain unchanged]
...
int main() {
    ...
    MPI_Scatterv(all_fishes, sendcounts, displs, MPI_FISH_TYPE, local_fishes, sendcounts[rank], MPI_FISH_TYPE, 0,
                 MPI_COMM_WORLD);
    ...
    // ... [rest of the logic for the simulation, objective function, barycenter, etc.]
    for (int step = 0; step < NUM_SIMULATION_STEPS; step++) {

        printf("Simulation step %d\n", step);
        simulationStep(local_fishes, sendcounts[rank]); // Use sendcounts[rank] as localFishCount
...
#pragma omp parallel for reduction(+ : localSum)
        for (int j = 0; j < sendcounts[rank]; j++) {  // Changed iterator name to j
            localSum += calculateDistanceFromOrigin(local_fishes[j].x, local_fishes[j].y);
        }
...
#pragma omp parallel for reduction(+ : localWeightSum, localDistanceSum)
        for (int j = 0; j < sendcounts[rank]; j++) { // Changed iterator name to j
            localWeightSum +=
                    local_fishes[j].weight * calculateDistanceFromOrigin(local_fishes[j].x, local_fishes[j].y);
            localDistanceSum += calculateDistanceFromOrigin(local_fishes[j].x, local_fishes[j].y);
        }
    ...
    MPI_Gatherv(local_fishes, sendcounts[rank], MPI_FISH_TYPE, all_fishes, sendcounts, displs, MPI_FISH_TYPE, 0,
                MPI_COMM_WORLD);
